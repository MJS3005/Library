...
$db->query("UPDATE `books` SET `status` = 'Нет в наличии; выдана ч/б № {$_POST['usersId']}' WHERE `id` = '{$_POST['booksId']}'");



$user = mysqli_fetch_all((mysqli_query($db, "SELECT * FROM users WHERE `id` = '{$_POST['usersId']}'")), MYSQLI_ASSOC);
$book = mysqli_fetch_all((mysqli_query($db, "SELECT * FROM `books` WHERE `id` = '{$_POST['booksId']}'")), MYSQLI_ASSOC);
        ...
        <div class="login-block">
            ...
            <form action="" method="post" class="registration-form">
                <div class="form-wrapper">
                    <ul class="login-list">
                        <li><label for="booksId">Артикул издания</label><input
                                type="text" name="booksId" id="booksId" required></li>
                        <li><label for="usersId">Номер читательского билета, на который выдается книга</label><input type="text" name="usersId"
                                id="usersId" required></li>
                        <li><label for="returnDate">Срок возврата книги</label><input type="text" name="returnDate"
                        id="returnDate" value = "<?php 
                        $returnDateU = time() + 60*60*3 + 60*60*24*7; 
